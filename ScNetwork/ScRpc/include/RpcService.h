...
protected:
	...
	SThread::CMutex*		m_pMutex;
	bool					m_bAutoKeepAlive;

};
//共享通道的， 并发送创建消息
template <class T>
int Scp::CRpcService::CreateServiceIf(T*&pNewService, int64 i64IdFrom, OUT_P int64& i64IdTo)
{
...
//独立客户端通道, 并发送创建消息
template <class T>
int Scp::CRpcService::CreateServiceIf(T*&pNewService, int64 i64IdFrom, OUT_P int64& i64IdTo, OUT_P string& strRemoteIp, OUT_P int &nRemotePort, string& strLocalIp, int& nLocalPort)
{
	T* pNewServiceTmp = new T();
	CRpcService *pRpcService = dynamic_cast<CRpcService*>(pNewServiceTmp);
	if (!pNewServiceTmp)
