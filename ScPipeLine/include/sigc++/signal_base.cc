...
void signal_impl::disconnect(void *pFun, void *pObj)
{
	iterator_type i = slots_.begin();
	for ( ;i != slots_.end(); /*i++*/)
	{
		if ( i->m_pFun == pFun && i->m_pObj == pObj)
		{
			iterator_type inext = i;
			inext++;
			slots_.erase(i);
			i = inext;
		}
...
signal_impl::iterator_type signal_impl::insert(signal_impl::iterator_type i, const slot_base& slot_)
{
  ...
  temp->m_pObj = slot_.m_pObj;
  temp->m_pFun = slot_.m_pFun;
...
void signal_base::disconnect(void *pFun, void *pObj)
{
	impl()->disconnect(pFun, pObj);
}
