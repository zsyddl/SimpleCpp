...
public:
	...
	int Open(char *pSrcCodec, char *pDesCode);
	void Close();

	int Trans(char *pSrc, char *pDes);

	int From(char *pSrc, char *pCodecName);
	int To(char *pDes, char *pCodecName);

protected:
	iconv_t m_iconv;;
	char m_szFromCodec[64];
	char m_szToCodec[64];
	char *m_pSrc;
	char *m_pDes;
};
